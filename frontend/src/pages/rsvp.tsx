      ...
      <div ref={heroPaneRef} className={`h-screen sticky top-0 p-6 lg:p-10 flex flex-col ${isDark ? 'bg-[var(--theme-primary)]' : 'bg-[var(--theme-bg)]'}`}>
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-6 lg:gap-10 min-h-0">
          <div ref={heroImageRef} className="h-[35vh] lg:h-auto lg:flex-[2] min-w-0 overflow-hidden shrink-0 relative">
            ...
            />
            <span className="absolute top-8 left-8 bg-black/20 text-[var(--theme-text)]/60 text-xs font-mono tracking-wider uppercase z-10 px-3 py-1.5">{info.prefilled_first_name ? `${info.prefilled_first_name}, you're invited` : "You're invited"}</span>
          </div>
                    ...
                    {info.landing_program?.length > 0 && (
                      <div>
                        {info.landing_program.map((item, i) => {
                          ...
                          return (
                          <div key={i} className={`${isEdge ? 'bg-[#020202]' : 'bg-[#020202]/80'} ${i > 0 ? 'border-t border-[var(--theme-border)]/30' : ''}`}>
                            <button
                              type="button"
